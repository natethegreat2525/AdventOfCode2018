...
for (let a = 0; a < 10000; a++) {
  oldStates[JSON.stringify(arr)] = a;
  let newState = new Array(arr.length);
  for (let i = 0; i < arr.length; i++) {
    newState[i] = new Array(arr[0].length).fill('');
  }
  ...
  arr = newState;
  if (oldStates[JSON.stringify(arr)]) {
    console.log(a, oldStates[JSON.stringify(arr)]);
    let tree = 0;
    let lmb = 0;
    for (let i = 0; i < arr.length; i++) {
      for (let j = 0; j < arr[i].length; j++) {
