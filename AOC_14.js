...
for (let i = 0; i < 1000000000; i++) {
  let tot = start[e1] + start[e2];
  let str = tot + '';
  let nums = str.split('').map(a => parseInt(a));
  for (let j = 0; j < nums.length; j++) {
    start.push(nums[j]);

    if (start.length > 6) {
      if (start.slice(start.length-6).join('') === num) {
        log(start.length-6);
        break;
      }
    }
  }
  e1 = (e1 + start[e1] + 1) % start.length;
  e2 = (e2 + start[e2] + 1) % start.length;
  //log(start);
}
//log(start.slice(num, num+10).join(''))
