...
for (let i = 0; i < arr.length; i++) {
  for (let j = 0; j < arr[i].length; j++) {
    let blk = arr[i][j];
    if ("<>v^".includes(blk)) {
      carts.push({i: i, j: j, dir: blk, turn: 0});
      if ("<>".includes(blk)) {
        arr[i][j] = '-';
      } else {
        arr[i][j] = '|';
...
function moveForward(cart) {
  let curArr = arr[cart.i][cart.j];
  //log(cart);
  //log(curArr);
  if (curArr === '-' || curArr === '|') {
    switch(cart.dir) {
    ...
    for (let a = 0; a < 3; a++) {
      let str = '';
      for (let b = 0; b < 3; b++) {
        str += arr[cart.i+a-1][cart.j+b-1];
      }
      log(str);
    }

  }
  //log(arr[cart.i][cart.j]);
  //log("ij " + cart.i + " " + cart.j)
}

let end = false;
while (!end) {
  carts.sort((a, b) => {
    return a.i * arr.length + a.j - (b.i * arr.length + b.j);
  });
  log(carts);
  if (carts.length === 1) {
    log(carts);
  ...
  for (let i = 0; i < carts.length; i++) {
    ...
    for (let j = 0; j < carts.length; j++) {
      if (j !== i) {
        if (cart.i === carts[j].i && cart.j === carts[j].j) {
          if (i > j) {
            carts.splice(i, 1);
            carts.splice(j, 1);
            i--;
