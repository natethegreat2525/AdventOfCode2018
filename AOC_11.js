...
let sarr = new Array(300);

for (let i = 0; i < 300; i++) {
  arr[i] = new Array(300).fill(0);
  sarr[i] = new Array(300).fill(0);
}
function conv(i, j) {
  let rid = i + 10;
  let pl = rid * j + sn;
  pl *= rid;
  pl = (Math.floor(pl / 100) % 10) - 5;
  return pl;
}
for (let i = 1; i < 301; i++) {
  for (let j = 1; j < 301; j++) {
    arr[i-1][j-1] = conv(i, j);
    if (i == 1 && j == 1) {
    sarr[i-1][j-1] = arr[i-1][j-1];
    } else {
      if (j == 1) {
        sarr[i-1][j-1] = arr[i-1][j-1] + sarr[i-2][j-1];
      } else if (i == 1) {
        sarr[i-1][j-1] = arr[i-1][j-1] + sarr[i-1][j-2];
      } else {
        sarr[i-1][j-1] = arr[i-1][j-1] + sarr[i-2][j-1] + sarr[i-1][j-2] - sarr[i-2][j-2];
      }
    }
  }
}
...
for (let i = 1; i < 301; i++) {
  for (let j = 1; j < 301; j++) {
    for (let sz = 1; sz < 300; sz++) {
      if (i-1+sz-1 >= 300 || j-1+sz-1 >= 300) {
        continue;
      }
      let left = i > 1 ? sarr[i-2][j-1+sz-1] : 0;
      let up = j > 1 ? sarr[i-1+sz-1][j-2] : 0;
      let diag = (i > 1 && j > 1) ? sarr[i-2][j-2] : 0;
      let sum = sarr[i-1+sz-1][j-1+sz-1] - left - up + diag;
      /*
      for (let a = 0; a < sz; a++) {
        for (let b = 0; b < sz; b++) {
          sum += arr[i-1+a][j-1+b];
