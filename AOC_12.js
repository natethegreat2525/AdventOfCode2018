...
let pad = 200;
let padval = new Array(pad).fill('.').join('');
let initState = padval + `##.#....#..#......#..######..#.####.....#......##.##.##...#..#....#.#.##..##.##.#.#..#.#....#.#..#.#` + padval;
let str = `#.#.. => .
..##. => .
...#. => .
..#.. => .
...
for (let i = 0; i < 180; i++) {
  ...
  for (let j = 0; j < state.length; j++) {
    ...
    for (let k = 0; k < arr.length; k++) {
      let pt = arr[k][0];
      let vl = arr[k][1];
      if (pt[0] === state[j-2] && pt[1] === state[j-1] && pt[2] === state[j-0] && pt[3] === state[j+1] && pt[4] === state[j+2]) {
        nw.push(vl);
        found = true;
        break;
      }
